...
class Obj{ //宣告一個類別，針對一個畫的圖案
    constructor(args){ //預設值，基本資料(物件的顏色、移動的速度、大小、初始位置....)
      // this.p = args.p ||{x:random(width),y:random(height)}//描述為該物件的初始位置，||(or)，當一個物件產生時，有傳給位置參數，則使用該位置參數，如果沒有傳參數，就以||後面設定產出
    ...
    update(){  //設定物件移動，移動的程式碼內容
      ...
      let delta = mouseV.sub(this.p).limit(this.v.mag()*2) //sub計算出滑鼠所在位置向量(mouseV)到物件向量(this.p)的距離
